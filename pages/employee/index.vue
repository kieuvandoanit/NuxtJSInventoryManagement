<template>
  <div class="relative shadow rounded overflow-hidden">
    <div class="flex items-center justify-between flex-column flex-wrap space-y-4 py-4 bg-white bg-white-900">
      <div class="relative ml-2">
        <div class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none">
          <svg class="w-4 h-4 text-white-500  text-white-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
        ...
        <input type="text" id="table-search-users"
          ...
          autocomplete="off" placeholder="Search for users">
      </div>
      <div>
        <NuxtLink
          to="/employee/create"
    ...
    <table class="w-full text-sm text-left rtl:text-right text-white-500">
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4"> {{ getEnumKeyByValue( EmployeeRole, item.role) }} </td>
          <td class="px-6 py-4">
            <div class="flex items-center">
              <div class="h-2.5 w-2.5 rounded-full bg-green-500 me-2"></div> {{ getEnumKeyByValue( EmployeeStatus, item.status) }}
            </div>
          </td>
          <td class="px-6 py-4">
            <NuxtLink :to="`/employee/${item.id}`" type="button"
